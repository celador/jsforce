...
Metadata.prototype.deployStatus = function (ids, callback) {
    var isArray = _.isArray(ids);
    var res = this._invoke("checkDeployStatus", { asyncProcessId: ids });
    return new AsyncResultLocator(this, res, isArray).thenCall(callback);
};
...
Metadata.prototype.deployRecentValidation = function (id, options, callback) {
    var self = this;
    return new DeployResultLocator(this, self._invoke("deployRecentValidation", id)).complete(callback);
};
...
AsyncResultLocator.prototype.deployStatus = function (callback) {
    var self = this;
    var meta = this._meta;
    return this.then(function (results) {
        var ids = _.isArray(results) ? _.map(results, function (res) { return res.id; }) : results.id;
        ...
        return meta.checkDeployStatus(ids);
    }).thenCall(callback);
};
...
AsyncResultLocator.prototype.pollDeploy = function (interval, timeout) {
    var self = this;
    var startTime = new Date().getTime();
    var poll = function () {
        var now = new Date().getTime();
        ...
        self.deployStatus().then(function (results) {
            var done = true;
            var resultArr = _.isArray(results) ? results : [results];
            for (var i = 0, len = resultArr.length; i < len; i++) {
                var result = resultArr[i];
...
AsyncResultLocator.prototype.complete = function (callback) {
    ...
    this.on('progress', function (result) {
        console.info('Status: ' + result.state);
    });
    var meta = this._meta;
    this.poll(meta.pollInterval, meta.pollTimeout);
    return deferred.promise.thenCall(callback);
};
AsyncResultLocator.prototype.completeDeploy = function (callback) {
    var deferred = Promise.defer();
    this.on('complete', function (results) {
        deferred.resolve(results);
    });
    ...
    this.on('progress', function (result) {
        if (result.status === 'Pending') {
            console.info('Deploy is Pending')
        } else if (result.status === 'InProgress') {
            // console.info('Components: ' + result.numberComponentsDeployed + '/' + result.numberComponentsTotal + ' e: ' + result.numberComponentErrors);
            if (result.numberTestsTotal > 0) {
                console.info('Tests: ' + result.numberTestsCompleted + '/' + result.numberTestsTotal + ' errors: ' + result.numberTestErrors + ' - ' + result.stateDetail);
            } else if (result.numberComponentsTotal > 0) {
                console.info('Components: ' + result.numberComponentsDeployed + '/' + result.numberComponentsTotal + ' errors: ' + result.numberComponentErrors + ' - ' + result.stateDetail);
            }  
        }
    });
    var meta = this._meta;
    this.pollDeploy(meta.pollInterval, meta.pollTimeout);
    return deferred.promise.thenCall(callback);
};
...
DeployResultLocator.prototype.complete = function (includeDetails, callback) {
    ...
    var meta = this._meta;
    return DeployResultLocator.super_.prototype.completeDeploy.call(this).then(function (result) {
