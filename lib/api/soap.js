...
/**
 * Call SOAP Api (Enterprise) endpoint
 * @private
 */
SoapApi.prototype._invokeE = function(method, message, schema, callback) {
    var soapEndpoint = new SOAP(this._conn, {
        xmlns: "urn:enterprise.soap.sforce.com",
        endpointUrl: this._conn.instanceUrl + "/services/Soap/c/" + this._conn.version
    });
    return soapEndpoint.invoke(method, message, { result: schema }).then(function(res) {
        return res.result;
    }).thenCall(callback);
...
/**
 * Call SOAP Api (Apex) endpoint
 * @private
 */
SoapApi.prototype._invokeA = function(method, message, schema, callback) {
    var soapEndpoint = new SOAP(this._conn, {
        xmlns: "http://soap.sforce.com/2006/08/apex",
        endpointUrl: this._conn.instanceUrl + "/services/Soap/s/" + this._conn.version
    });
    return soapEndpoint.invoke(method, message, schema).thenCall(callback);
};
 ...
 */
SoapApi.prototype.executeAnonymous = function(apexcode, callback) {
    return this._invokeA("executeAnonymous", { apexcode: apexcode }, null, callback);
};
Schemas.ExecuteAnonymous = {
    Body: Schemas.ExecuteAnonymousBody,
    Header: Schemas.ExecuteAnonymousHeader
}
Schemas.ExecuteAnonymousBody = {
...
Schemas.ExecuteAnonymousHeader = {
    debugLog: 'string'
};
